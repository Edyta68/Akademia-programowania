...
private:
		...
		for (int k = bind + 1; k < bind + 7; k++)
		{
			...
			if (k % players_count == player_index&& players[player_index].round)
			{
				if (betOn)
				{
					...
					cin >> action;
					while ((action != FLOP && action != BET_or_CALL) || (action == BET_or_CALL && players[player_index].money <= 0))
					{
	...
	void addWinnigNote()
	{
        std::cout << std::endl;
			...
			for(int e=0; e<players_count;e++)
			{
                if(players[e].goodToGo && e!=roundWinner)
                {
                    std::cout<<players[e].name<<":"<<std::endl;
                    printWinningHand(e);
                }
